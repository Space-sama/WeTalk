...
export default class RegistrationForm {
    ...
    events() {
        ...
        this.username.addEventListener("keyup", () =>{
            this.isDiffrent(this.username, this.usernameHandler)
        })

        this.email.addEventListener("keyup", () =>{
            this.isDiffrent(this.email, this.emailHandler)
        })

        this.pass.addEventListener("keyup", () =>{
            this.isDiffrent(this.pass, this.passwordHandler)
        })
        ...
        this.pass.addEventListener("blur", () =>{
            this.isDiffrent(this.pass, this.passwordHandler)
        })
    ...
    insertValidationElements() {
        this.allFields.forEach(function(elm) {
            elm.insertAdjacentHTML('afterend', '<div class="alert alert-danger small liveValidateMessage"></div>')
        })
    }
    ...
    passwordHandler() {
        this.pass.errors = false 
        this.passwordImmediately()
        clearTimeout(this.pass.timer)
        this.pass.timer = setTimeout(() => this.passwordAfterDelay(), 800)
    }
    ...
    passwordImmediately() {
        if (this.pass.value.length > 50) {
            this.showValidationError(this.pass, "Password cannot exceed 50 characters.")
        }

        if (!this.pass.errors) {
            this.hideValidationError(this.pass)
        }
    }

    passwordAfterDelay() {
        if (this.pass.value.length < 12) {
            this.showValidationError(this.pass, "Password must be at least 12 characters.")
        }
    }
    ...
    isDiffrent(el, handlerUsername) {
        if (el.previousValue != el.value) {
            handlerUsername.call(this)
        }
